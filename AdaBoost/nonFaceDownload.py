                ...
                while i < len(search_keyword):      # 2.for every main keyword
                    ...
                    if arguments['related_images']:
                        ...
                        for key, value in tabs.items():
                            ...
                            self.create_directories(main_directory, arguments['image_directory'],arguments['thumbnail'])
                            self._get_all_items(new_raw_html, main_directory,arguments['image_directory'], limit,arguments)
...
haarModelPath=r'E:\Projects\cpp\opencv\build\etc\haarcascades\haarcascade_frontalface_default.xml'

datasetDir=r'E:\dataset'
if __name__ == '__main__':
    records = user_input()
    for arguments in records:
        response = googleimagesdownload()
        arguments['keywords'] = 'wallpaper'
        arguments['related_images'] = True
        arguments['output_directory']=datasetDir
        arguments['image_directory']='noFace'
        arguments['print_paths'] =True
        arguments['limit'] = 6000
        arguments['chromedriver'] = r'C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe'
        #absolute_image_paths = response.download(arguments)
        print("download complete")
        #删除人脸图片
        list_dirs = os.walk(datasetDir+'/noFace')
        facePatterns = cv2.CascadeClassifier(haarModelPath)
        for root, dirs, files in list_dirs:
            for file in files:
                ...
                try:
                    ...
                    faces=facePatterns.detectMultiScale(gray)
                    if len(faces)<=0:
                        #将图片从长的方向分为2份
                        shotname,extension = os.path.splitext(file)
                        rows,cols,depth=image.shape
                        if cols>rows:
                            image1=image[:,0:cols//2]
                            cv2.imwrite(os.path.join(root,shotname+'1.jpg'),image1)
                            image2=image[:,cols//2:-1]
                            cv2.imwrite(os.path.join(root,shotname+'2.jpg'),image2)
                        else:
                            image1=image[0:rows//2,:]
                            cv2.imwrite(os.path.join(root,shotname+'1.jpg'),image1)
                            image2=image[rows//2:-1,:]
                            cv2.imwrite(os.path.join(root,shotname+'2.jpg'),image2)
                except:
                    pass
                finally:
